...
- Services orchestrate external verification through a dedicated provider client when implemented; a stub or sandbox client can be used for tests. Decision 0002 supersedes the original mandatory application-port structure.
...
- Provider-specific SDK types must not leak into entities, API DTOs, or controllers.
