...
public class GraduationRequirementController {
    ...
    @Operation(
        summary = "내 졸업요건 조회",
        description = "[인증 O] 입학연도와 활성 전공별 졸업요건 및 필수과목을 조회합니다.",
        security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200",
            description = "졸업요건 조회 성공",
            content = @Content(
                mediaType = "application/json",
                array = @ArraySchema(
                    schema = @Schema(implementation = GraduationRequirementResponse.class)
                ),
                examples = @ExampleObject(value = """
                    [
                      {
                        "entityId": "00000000-0000-0000-0000-000000000401",
                        "admissionYear": 2024,
                        "majorType": "PRIMARY",
                        "department": {
    ...
    @GetMapping
    public List<GraduationRequirementResponse> getRequirements(
        @AuthenticationPrincipal Jwt jwt
    ) {
        return graduationRequirementService.getRequirements(
